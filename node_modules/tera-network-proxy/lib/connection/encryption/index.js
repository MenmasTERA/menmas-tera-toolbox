...
const { TeraCrypto, hasPadding } = require('tera-network-crypto');

function getCrypto(protocolVersion, key) {
    ...
    return new TeraCrypto(data, protocolVersion);
}
...
class Session {
    ...
    init() {
        ...
        xorKey(t2, t1, t2);
        this.decryptor = getCrypto(this.protocolVersion, t2);
        shiftKey(t1, s2, this.useOldConstants ? -79 : -41);
        this.decryptor.decrypt(t1);
        this.encryptor = getCrypto(this.protocolVersion, t1);
        this.preparer = this.decryptor;
    }

    hasPadding(code) {
        return hasPadding(this.protocolVersion, code);
    }

    prepareFromClient(data) {
        return this.preparer.prepare(data, false);
    }

    prepareToClient(data) {
        return this.preparer.prepare(data, false);
    }

    prepareFromServer(data) {
        return this.preparer.prepare(data, false);
    }

    prepareToServer(data) {
        return this.preparer.prepare(data, true);
    }
